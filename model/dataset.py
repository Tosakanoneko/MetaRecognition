...
class DataSet(tordata.Dataset):
    def __init__(self, data_cfg, target=None, dataset_root=None):
        """
            seqs_info: the list with each element indicating 
                            a certain gait sequence presented as [label, type, view, paths];
        """
        self.__dataset_parser(data_cfg, target)
    ...
    def __dataset_parser(self, data_config, target, dataset_root=None):
        ...
        if target is not None:
            target_set = [label for label in target_set if label in label_list]
            self.seqs_info = get_seqs_info_list(target_set)
        else:
    ...
    #                         seqs_info_list.append([*seq_info, seq_dirs])
    #         return seqs_info_list
    #     if 'dataset_partition' not in data_config.keys() or data_config['dataset_partition'] is None or not os.path.exists(data_config['dataset_partition']):
    #         self.seqs_info = get_seqs_info_list(os.listdir(dataset_root))
    #     else:
    #         with open(data_config['dataset_partition'], "rb") as f:
    #             partition = json.load(f)
    #         train_set = partition["TRAIN_SET"]
