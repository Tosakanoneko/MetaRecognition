...
class Operation:
	...
	def finger_search(self, label, frame, fpimg_label):
		...
		if self.finger_id == 0:
			finger_identity = "卢泓睿"
			img_path = os.path.join(script_dir, 'finger\\lhrfp.bmp')
		elif self.finger_id == 1:
			finger_identity = "郑贝来"
			img_path = os.path.join(script_dir, 'finger\\zblfp.bmp')
		elif self.finger_id == 2:
			finger_identity = "林明杰"
			img_path = os.path.join(script_dir, 'finger\\lmjfp.bmp')
		
		
		# 计算纵横比
		img = Image.open(img_path)
		scale_factor = 0.95  # 缩小到95%的大小
		aspect_ratio = img.width / img.height
		new_width = int((min(frame_width, frame_height * aspect_ratio)) * scale_factor)
		new_height = int((new_width / aspect_ratio) * scale_factor)	
		# img = img.resize((new_width, new_height), Image.LANCZOS)
		img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)
		imgtk = ImageTk.PhotoImage(image=img)
		fpimg_label.config(image=imgtk)
		fpimg_label.imgtk = imgtk
		
		label.config(text=f"人物:{finger_identity}", font=("宋体", 13))
...
def search_fingerprint_on_device(session, as608_lib, label, frame, fpimg_label):
	...
	if session.image_2_tz(1) != as608_lib.OK:
		...
		return False
	print("Searching......")
	if session.finger_search(label, frame, fpimg_label) != as608_lib.OK:
		return False
	return True

def search_fingerprint(session, as608_lib):
	"""Get a finger print image, template it, and see if it matches!"""
	print("Waiting for image...")
	while session.get_image() != as608_lib.OK:
		pass
	...
	if session.image_2_tz(1) != as608_lib.OK:
		return False
	# print("Searching...")
	# if session._finger_search(label) != as608_lib.OK:
	# 	return False
	return True
...
def delete_all(session,as608_lib):
	i = session.empty_library()
	if i == as608_lib.OK:
		print("Delete All")
	else:
		print("Delete Error")
